...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const fullDay = Math.ceil(time / (1000 * 60 * 60 * 24));
  return fullDay + 1;
}
...
function formatDate(date) {
  ...
  const seconds = dates.getUTCSeconds().toString().padStart(2, '0');
  const morningEvening = hours >= 12 ? 'PM' : 'AM';
  hours %= 12;
  hours = hours || 12;
  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${morningEvening}`;
}
...
function getWeekNumberByDate(date) {
  ...
  let weekCount = Math.ceil((date - startOfYear) / WEEK_IN_MS);
  weekCount += startOfYear.getDay() === date.getDay() ? 1 : 0;
  return weekCount;
}
...
function getNextFridayThe13th(date) {
  ...
  while (currentMonth < 12) {
    const checkDate = new Date(currentYear, currentMonth, 13);
    if (checkDate.getDay() === 5 && checkDate >= startDate) {
      return checkDate;
    }
    currentMonth += 1;
    if (currentMonth === 12) {
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const beginDate = new Date(period.start.split('-').reverse().join('-'));
  const finishDate = new Date(period.end.split('-').reverse().join('-'));
  const workDates = [];
  const current = new Date(beginDate);
  while (current <= finishDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (current > finishDate) return workDates;
      const day = String(current.getDate()).padStart(2, '0');
      const month = String(current.getMonth() + 1).padStart(2, '0');
      const year = current.getFullYear();
      workDates.push(`${day}-${month}-${year}`);
